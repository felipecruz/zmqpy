...
from zmqpy.eventloop.ioloop import IOLoop
from zmqpy.eventloop import stack_context
...
from zmqpy.utils.strtypes import bytes, unicode, basestring
...
class ZMQStream(object):
    ...
    For use with zmqpy.eventloop.ioloop
    ...
    * **stop_on_send():**
    ...
    def send(self, msg, flags=0, copy=True, track=False, callback=None):
        ...
        See zmqpy.socket.send for details.
        """
    ...
    def send_multipart(self, msg, flags=0, copy=True, track=False, callback=None):
        ...
        See zmqpy.socket.send_multipart for details.
        """
    ...
    def send_string(self, u, flags=0, encoding='utf-8', callback=None):
        ...
        See zmqpy.socket.send_unicode for details.
        """
    ...
    def send_json(self, obj, flags=0, callback=None):
        ...
        See zmqpy.socket.send_json for details.
        """
        if jsonapi is None:
    ...
    def flush(self, flag=zmqpy.POLLIN|zmqpy.POLLOUT, limit=None):
        ...
        def update_flag():
            ...
            return flag & zmqpy.POLLIN | (self.sending() and flag & zmqpy.POLLOUT)
        flag = update_flag()
        if not flag:
            # nothing to do
            return 0
        ...
        while events and (not limit or count < limit):
            s,event = events[0]
            if event & zmqpy.POLLIN: # receiving
                self._handle_recv()
                count += 1
                if self.socket is None:
                    # break if socket was closed during callback
                    break
            if event & zmqpy.POLLOUT and self.sending():
                self._handle_send()
                count += 1
                if self.socket is None:
                    # break if socket was closed during callback
    ...
    def _handle_recv(self):
        ...
        if self._flushed:
            return
        try:
            msg = self.socket.recv_multipart(zmqpy.NOBLOCK, copy=self._recv_copy)
        except zmqpy.ZMQError as e:
            if e.errno == zmqpy.EAGAIN:
                # state changed since poll event
                pass
            else:
                logging.error("RECV Error: %s"%zmqpy.strerror(e.errno))
        else:
