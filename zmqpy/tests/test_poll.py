...
class TestPoll(BaseZMQTestCase):
    ...
    def test_pubsub(self):
        ...
        s2.setsockopt(zmqpy.SUBSCRIBE, asbytes(''))
...
class TestSelect(BaseZMQTestCase):
    ...
    def test_pair(self):
        ...
        rlist, wlist, xlist = zmqpy.select([s1, s2], [s1, s2], [s1, s2])
    ...
    def test_timeout(self):
        ...
        s1, s2 = self.create_bound_pair(zmqpy.PAIR, zmqpy.PAIR)
        tic = time.time()
        r,w,x = zmqpy.select([s1,s2],[],[],.002)
        toc = time.time()
        self.assertTrue(toc-tic < 1)
        self.assertTrue(toc-tic > 0.001)
        tic = time.time()
        r,w,x = zmqpy.select([s1,s2],[],[],.25)
        toc = time.time()
        self.assertTrue(toc-tic < 0.5)
        self.assertTrue(toc-tic > 0.2)
